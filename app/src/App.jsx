...
export default function App() {
  ...
  function getProjectPayload() {
    return {
      version: "V5.8 Preview Comercial Fiel com Snapshots Reais",
  ...
  function buildPreview3DPages(snapshotMap = new Map()) {
    const pages = [];
    pages.push({
      id: "preview-cover",
      type: "cover",
  ...
  async function makePreview() {
    ...
    try {
      ...
      setModal({ type: "preview-3d", pages: buildPreview3DPages(snapshotMap) });
    } catch (error) {
      setModal({
        type: "preview-error",
    ...
    <div className="app-shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <strong>Diagramador Picmimos V5.8 Preview Comercial Fiel</strong>
            <span>Pré-visualização fiel ao editor, com capa e páginas renderizadas a partir do projeto real</span>
          </div>
        </div>
...
function Preview3D({ pages, photoMap }) {
  ...
  return (
    <div className="preview3d-shell preview58-shell">
      <div className="preview3d-config-badges premium-scene-badges" aria-label="Configuração usada no preview V5.8">
        <span>{activeMeta.model}</span>
        <span>{activeMeta.format}</span>
        <span>{activeMeta.lombada}</span>
        <span>Snapshots reais do editor</span>
      </div>

      <div className="preview3d-motion-controls premium-scene-controls" aria-label="Controles de visualização V5.8">
      ...
      <div className="preview3d-premium-wrap preview58-wrap">
        <SnapshotAlbumPreview page={renderPage} coverPage={coverPage} mode={viewMode} zoom={zoom} hasCoverPhoto={hasCoverPhoto} />
      ...
      <p className="preview3d-stable-note premium-scene-note">
        V5.8 preview comercial fiel: a prévia usa snapshots reais do editor para mostrar capa, páginas, lombada e verso com mais precisão.
      </p>
    </div>
  );
}
    ...
    <div className="modal-backdrop">
      <div className={`modal-card ${modal.type === "preview-3d" ? "preview-3d-card" : ""}`}>
        {modal.type === "saved" && (
          <>
            <h2>Projeto salvo</h2>
            <p>O projeto foi salvo no navegador para teste da V5.8.</p>
          </>
        )}
        {modal.type === "preview-loading" && (
          <>
            <h2>Pré-visualização V5.8</h2>
            <div className="preview58-loading">
              <strong>Gerando snapshots reais do editor...</strong>
              <span>Fotos, textos, crop e zoom estão sendo capturados antes de abrir a prévia.</span>
            </div>
          </>
        )}
        ...
        {modal.type === "preview-3d" && (
          <>
            ...
            <p>Use as setas na tela ou as teclas ← e → para folhear. Os botões Capa, Aberto, Lombada e Verso alternam poses comerciais estáveis usando os snapshots do editor.</p>
          </>
        )}
        {modal.type === "finalize" && (
          <>
